...
> 版本：2026-07-07（状态更新：2026-08-13）  
> 状态：**已落地**（微信 OAuth / 绑手机 / 家长入口已接入；具体环境变量与联调见 `backend/.env.example` 与 API 文档）
...
flowchart TD
  ...
  E -->|否| G[跳转 m.jnao.com 绑手机]
  G --> G2[用户回 Jnao 再 OAuth]
  G2 --> E
  F --> H{account_ready?}
  G --> H
  H -->|否| I[完善资料+密码]
...
|------|------|----------|
| S0 | 无 session | OAuth |
| S1 | 有 session，未绑手机 | 跳转 [公司绑手机页](https://m.jnao.com/home/member/bindmobile.html)，填完后回 Jnao 再 OAuth |
...
| GET | `/api/auth/wechat/callback` | code 换 openid；有 mobile 进 Jnao，无 mobile **302 到公司绑手机页** | ✅ |
| POST | `/api/auth/wechat/bind-phone` | Jnao 内置绑手机（仅 `WECHAT_BIND_MOBILE_URL=` 空时启用） | 备用 |
| POST | `/api/auth/wechat/send-bind-sms` | 内置绑手机发验证码 | 备用 |

**缺手机号 — 跳转公司绑手机（默认）：**

```text
https://m.jnao.com/home/member/bindmobile.html?redirect={Jnao登录页}
```

用户在公司页完成绑手机/注册后，回到 `https://jnaosoft.cn/pages/login/index?from=mp` 再次 OAuth → **强制刷新 `ys_wx_member`** → 按 openid 取 mobile → 登录 Jnao。

**回调成功（已有 mobile）302 到前端：**
...
| 场景 | 是否用 Jnao 短信 | 说明 |
|------|------------------|------|
| **浏览器**家长验证码登录/注册 | ✅ **保留** | `POST /api/auth/sms/*`，暂 `SMS_PROVIDER=mock`，**后期单独接阿里云** |
| **浏览器**家长密码登录 | — | 不涉及短信 |
| **微信内**缺手机号 | ❌ | 跳转 [m.jnao.com 绑手机页](https://m.jnao.com/home/member/bindmobile.html)，不经 Jnao 发短信 |
| Jnao 内置 `bind-phone`（备用） | ✅ | 仅 `WECHAT_BIND_MOBILE_URL=` 空时；与浏览器共用 SMS 通道 |

**当前：** 开发/内测继续 mock（`88888`）。**以后：** 只为浏览器登录/注册配置一套阿里云签名+模板，与微信绑手机、公司老系统短信互不混用。
...
# 微信内缺手机号 → 跳转公司绑手机（默认 m.jnao.com；设为空字符串则改用 Jnao 内置绑手机页）
WECHAT_BIND_MOBILE_URL=https://m.jnao.com/home/member/bindmobile.html
WECHAT_BIND_MOBILE_RETURN_PARAM=redirect
# WECHAT_BIND_MOBILE_RETURN_URL=https://jnaosoft.cn/pages/login/index?from=mp
```
...
### 8.3 浏览器验证码短信（暂 mock，后期阿里云）

**仅用于**非微信浏览器：`/api/auth/sms/send`（scene=`login` / `register`）。
...
# 后期上线浏览器验证码 — 单独一套阿里云（与微信绑手机、公司 m.jnao 无关）
# SMS_PROVIDER=aliyun
...
可选：`SMS_PROVIDER=company` + `COMPANY_SMS_*` 走公司 HTTP 代理（非默认方案）。

生产环境上线浏览器验证码前需完成阿里云接入；**未接入前**可继续 mock（仅 `JNAO_ENV=development`）或暂不开放 SMS 登录入口。
...
| 4 | 阿里云短信（**浏览器验证码**，后期） | SignName + TemplateCode + AccessKey |
...
| **Phase 3** | 浏览器验证码接 **阿里云**（单独配置） | 阿里云参数 |
