...
4. 身份入口：`ensureParentUser`（家长）/ `ensureChildUser`≈`ensureStudentUser`（学生）；`getParentUserId` / `getStudentUserId` 读槽位
5. 新代码优先按域引入，例如：`import { sendGuideMessage } from '@/utils/api/guide.js'`
