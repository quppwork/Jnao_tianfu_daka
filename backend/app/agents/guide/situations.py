...
def resolve_situation(ctx: GuideContext) -> tuple[str, str]:
    ...
    优先级：need_assessment > training_done / in_progress
    > sparse_return > ready_to_train
    """
    ...
    if ctx.today.exists and ctx.today.has_started:
        ...
        if (
            ctx.today.item_count > 0
            and ctx.today.done_count >= ctx.today.item_count
        ):
            return "training_done", "growth"
...
def template_welcome(
    ...
    *,
    ...
    if talent:
        text = text.replace("{talent}", str(talent))
    return text
